...
They had a previously successful product called ProofMe and wanted to embed that project into their new application. Embedding the app was easier said than done because they were completely different architectures. MFactor was a React-based SPA application, and ProofMe was a complex MVC .NET app with file conversion and rendering logic. Unfortunately, the entire team that initially worked on ProofMe had left.

The first day I found out that they had _just_ gotten the product working in a developer environment, and with two months to go for our major launch, I had concerns about meeting such a tight deadline.
...
- Can I render images using the same pattern Google Maps does?
- Can I convert images to that format?
- Can I have drawings and comment placement tracked using a coordinate system?

Using [https://leafletjs.com/](Leaflet) and React, I quickly built a demo showing that this could work, and the Monday after my flight home, I showed the CTO the demo of what I had made.
...
We ended up using my solution for a few reasons. The big one is that because the new product was built in React, we could easily embed the React-based proofer as a component. Since we were building everything from scratch, we also had a deeper understanding of everything, so adding new features was far less complex. Because we were building off of a popular mapping library, we could launch with many features that have existed in the mapping space for a long time that would have had to be built completely custom.

We launched two months later at Adobe MAX with video and image comparison with a slider to compare differences and a highlight changes function. We even had frame-by-frame annotation, allowing directors or CG artists to get feedback on particular effects for a specific moment.
